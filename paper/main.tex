%%%% main.tex (workshop submission)
%%%% Carry-Over Lottery Allocation: family + manipulation bound
%%%% IJCAI-ECAI 2026 CFD Workshop
...
% Guarded for engines without \pdfinfo (e.g., XeTeX-based tectonic).
\makeatletter
...
\citet{munro2020notanking} formalise the central tension as an impossibility. Define a \emph{No-Tanking Condition} (NTC): no team can improve its expected outcome by losing. Any mechanism mapping end-of-season records to draft probabilities, while giving worse-record teams strictly better odds, must violate NTC. Their proof constructs a minimal counterexample (two teams, both eliminated, identical records, final game between them), so the loser receives better draft odds and both teams prefer to lose. Only a uniform lottery (equal odds for all non-playoff teams) satisfies NTC, abandoning differential reward. The 2026 ``3-2-1'' proposal takes precisely this route, with adjustments for the play-in tournament \citep{highley2026solvable5}.

Carry-Over Lottery Allocation (\cola{}) sidesteps the impossibility by replacing single-season records with multi-year playoff history as the basis for draft priority \citep{highley2026cola}. Each non-playoff team accumulates lottery tickets over consecutive seasons; success in the playoffs or the draft itself diminishes a team's accumulated tickets. The single-season manipulation channel that drives the Munro-Banchio counterexample becomes ineffective: no single-season choice materially shifts the multi-year ticket pool. \cola{} is a family of mechanisms rather than a single design; the original paper develops one concrete instance (\emph{Classic \cola{}}), and subsequent work introduces variants that occupy different points on the explainability--robustness frontier.

\paragraph{Contributions.} We address three questions in this paper.
\begin{itemize}
    \item \textbf{(Section~\ref{sec:bound}) An analytic manipulation bound.} \cola{}'s incentive-compatibility argument rests on five assumptions; four hold by construction, the fifth is supported empirically. We derive a closed-form upper bound of approximately $4\%$ on the worst-case probability gain from pool manipulation, with explicit sensitivity to the underlying index-distribution parameters. The bound contains the simulation-observed extremes (a $3\%$ maximum reported by \citet{highley2026cola} across $1{,}000$ synthetic seasons) with $1.3\times$ headroom and provides the first parameter-explicit ceiling.
    \item \textbf{(Section~\ref{sec:family}--\ref{sec:bound}) A family-level perspective.} We present the \cola{} family as five variants on a shared scaffold (Section~\ref{sec:family}), including \emph{Capped \cola{}} \citep{highley2026solvable5} which introduces a stockpile cap to bound the per-series cost of advancing in the playoffs. We prove an analytic per-series bound (Lemma~\ref{lem:capped-bound}): under cap $\Max$, no playoff series win ever costs a team more than $0.3 \cdot \Max$ tickets ($45$ at $\Max = 150$).
    \item \textbf{(Section~\ref{sec:fairdiv}) A bridge to the axiomatic literature.} \citet{coreno2022axiomatic} prove that no draft rule simultaneously satisfies Respect for Priority (RP), Envy-Free up to One Object (EF1), and Strategy-Proofness (SP). Their result is single-period; \cola{}'s multi-year structure raises an open question about whether the axioms lift. We surface a preliminary mapping and identify what would need to be shown to embed \cola{} in the axiomatic literature.
\end{itemize}

A 26-season historical backtest of all five variants (Section~\ref{sec:empirical}) calibrates the bounds against real NBA data, with the Philadelphia 76ers' 2013--2017 ``Process'' as a case study in how multi-year diminishment dismantles repeat-tanking strategies that the current weighted lottery rewards.
...
\citet{munro2020notanking} establish that any mechanism mapping end-of-season records to draft probabilities, while giving worse teams strictly better odds, must violate NTC. The proof constructs a minimal counterexample: two eliminated teams with identical records and a final game between them. The loser finishes with a worse record and therefore receives better draft odds; both teams prefer to lose. Only the uniform lottery satisfies NTC, but uniform odds abandon the goal of differentially rewarding the weakest teams.

The authors propose \emph{T-IC} (Truncated Incentive Compatibility) as an alternative. T-IC tracks each team's conditional probability of finishing last and freezes allocations at the moment IC would first be violated. The mechanism is optimal in theory but has three practical issues. First, it is computationally expensive and opaque to fans. Second, calculating when IC would first be violated requires precise team-level valuations of playoff-versus-draft trade-offs that the league does not have access to. Third, the truncation timing assumes teams develop tanking incentives only after elimination, but some teams have an incentive to tank from the first game of the season (rebuilds with multi-year horizons), which the truncation never reaches.

The 2026 ``3-2-1'' reform proposal \citep{highley2026solvable5} adopts a different escape: it expands the lottery to 16 teams with bounded ball allocation (bottom 3 by record receive 2 balls, the next 7 receive 3 balls, the play-in losers receive 1 or 2 balls based on seed), totalling 37 balls and producing per-team odds of $2.7\%$, $5.4\%$, or $8.1\%$. The proposal effectively approximates a uniform lottery within an adjustment for the play-in tournament, achieving NTC at the cost of weakening differential reward. It expires in 2029, leaving the design space open for re-evaluation.

\subsection{Preference Manipulation and the Axiomatic Question}
\label{sec:fairdiv}

\citet{coreno2022axiomatic} address a complementary manipulation channel: teams misreporting which players they prefer. They prove that no allocation rule simultaneously satisfies:
\begin{itemize}
...
The constituent axioms trace to peer-reviewed sources. RP extends Svensson's ``weak fairness'' from queue allocation to draft priority \citep{svensson1994allocation}. EF1 originates with \citet{lipton2004approximately} for indivisible-goods allocation in computer science settings, and is adapted to the assignment problem by \citet{budish2011combinatorial} in economics.

The Coreno-Balbuzanov result is single-period: it characterises one-shot allocations of indivisible items. \cola{} is multi-period: a team's priority in year $t$ is a function of its outcomes in years $t-1, t-2, \ldots$. Whether the axioms lift to this setting is open. We sketch a preliminary mapping below; we do not prove the lift, and we treat each direction as a question worth resolving rather than a theorem.

\paragraph{RP under \cola{}.} Within a single season, a Classic \cola{} priority ordering is induced by the lottery index $L_i$. RP requires that higher-priority teams receive weakly better allocations. Classic \cola{} achieves this in expectation: $\E[\text{pick rank} \mid L_i] $ is monotone decreasing in $L_i / P^{(t)}$. For per-realisation RP (which is what Coreno-Balbuzanov require), the lottery's randomisation could violate the property: a team with $L_i > L_j$ may receive a worse pick than $j$ in any single draw. The standard fix in fair-allocation literature is to consider RP under the induced expected allocation, but no formal lift has been established.

\paragraph{EF1 under \cola{}.} The graduated diminishment schedule (champion: $\rho = 1.0$; finals: $0.75$; conf finals: $0.5$; second round: $0.25$; first round: $0$) preserves a weak EF1-like property: after the playoff outcome is realised, no team prefers another team's running index to its own. Whether this constitutes EF1 in the formal sense (envy after the removal of a single item from the envied bundle) requires identifying the relevant ``items.'' One natural reading: the items are the picks within the lottery, and EF1 requires no team to prefer another's lottery outcome after removing the most-preferred pick. We have not verified this claim formally and offer it as a target for the workshop's audience.

\paragraph{SP under \cola{}.} This is the manipulation-resistance question Section~\ref{sec:bound} addresses analytically for the pool-manipulation channel under Classic \cola{}, and Lemma~\ref{lem:capped-bound} addresses for the per-series channel under Capped \cola{}.
...
The variants below trade off explainability, robustness, and per-series tanking exposure differently. Table~\ref{tab:variants} summarises the comparison.
...
\bottomrule
\end{tabular}
\caption{The \cola{} family. ``sw $= 0$'' is shorthand for ``zero playoff series wins this season.'' Capped \cola{}'s exclusion-rule pool removes teams that won a top-5 lottery pick last season or a playoff series this/last season. Each variant occupies a distinct point on the explainability--robustness--exposure frontier; the manipulation bound (Theorem~\ref{thm:bound}) is established for Classic and the per-series bound (Lemma~\ref{lem:capped-bound}) for Capped.}
\label{tab:variants}
\end{table*}
...
The gain is maximised when $L_i$ is maximised and $\Delta / P$ is maximised. \emph{Bounding $p_{\max}$:} the highest-index team has $L_{\max} = T_{\max} \cdot \alpha$. The minimum pool size occurs when one team has $T_{\max}$ years of accumulation and the remaining $n - 1$ teams each have the minimum $\alpha$: $P_{\min} = (T_{\max} + n - 1) \cdot \alpha$. Thus $p_{\max} \leq T_{\max} / (T_{\max} + n - 1)$. \emph{Bounding $\Delta / P$:} the team being pushed out has at most $T_{\text{boundary}} \cdot \alpha$ tickets, and the team replacing it has at least $\alpha$, so $\Delta_{\max} = (T_{\text{boundary}} - 1) \cdot \alpha$. In steady state, $P \approx n \cdot \bar{k} \cdot \alpha$. Combining via the first-order approximation yields the stated bound.
\end{proof}

\paragraph{Empirical calibration.} We instantiate the bound with $T_{\max} = 10$ (the longest steady-state non-playoff stretch in our 26-season backtest, observed for the Sacramento Kings prior to their 2022--23 first-round appearance), $T_{\text{boundary}} = 5$ (a conservative ceiling on the consecutive non-playoff years a near-boundary team can accumulate before improving enough to clear the boundary), $n = 14$ (the structural count of NBA non-playoff teams), and $\bar{k} = 3.1$ (an average team has roughly three years of carry-over before reset, matching the original paper's observation). These yield
\begin{equation*}
G_{\max} \leq \frac{10 \cdot 4}{23 \cdot 14 \cdot 3.1} \approx 4.0\%,
\end{equation*}
which contains the simulation-observed maximum of $3.0\%$ across $1{,}000$ seasons \citep{highley2026cola} with $1.3\times$ headroom. An independent 200-season Bradley-Terry simulation we ran confirms the bound, with observed maximum gain of $1.70\%$.
...
Three patterns emerge. Sub-100 caps over-compress the lottery: at $\Max = 75$, more than six teams sit at the cap on average and the rank-1 vs.\ rank-5 separation collapses to under one ticket, destroying the priority signal the family inherits from Classic. Caps above 175 erase the cap's purpose: fewer than one team sits at the cap on average, and the per-series exposure approaches the unbounded behaviour of Classic. The published $\Max = 150$ choice produces fewer than two teams at the cap in 19 of 21 stable seasons and at most three in any single season, matching the stated headline of ``no more than three teams at the maximum at a time, and on average fewer than two'' \citep{highley2026solvable5}.
...
\begin{itemize}
    ...
    \item \emph{Axiomatic embedding.} The lift from \citet{coreno2022axiomatic}'s axioms (RP, EF1, SP) to multi-period priority mechanisms remains open. Section~\ref{sec:fairdiv} sketches the mapping; a formal verification would embed \cola{} in the existing axiomatic literature on assignment mechanisms and clarify which one-period impossibilities transfer to multi-period settings. We see this as the most natural follow-on for the workshop's audience.
\end{itemize}
